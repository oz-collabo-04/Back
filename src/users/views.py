...
class NaverLoginCallbackAPIView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        # 네이버에서 `code`와 `state` 매개변수로 리디렉션되었을 때 처리
    ...
    def get_naver_access_token(self, code, state):
        ...
        if response.status_code != 200:
            raise PermissionDenied("네이버에서 액세스 토큰을 가져오는 데 실패했습니다.")

        return response.json()

    def get_naver_user_info(self, access_token):
        # 네이버 사용자 정보를 가져오기 위한 헤더 설정
        headers = {
        ...
        if response.status_code != 200:
            raise PermissionDenied("네이버에서 사용자 정보를 가져오는 데 실패했습니다.")

        return response.json()

    def create_or_update_user(self, user_info):
        # 네이버 사용자 정보에서 필요한 필드 추출
        ...
        phone_number = user_info.get("response", {}).get("mobile")
        ...
        if user:
            ...
            user.gender = gender or user.gender  # 성별이 제공되지 않으면 기존 값 유지
            # 네이버 전화번호는 업데이트안하고 카카오 전화번호는 업데이트되게 설정함/ 정확성이 다를듯하여..
            # if phone_number:
